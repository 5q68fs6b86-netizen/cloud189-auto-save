...
推荐直接使用页面顶部的 **搜索并创建**：选择资源后，通常只需确认订阅名称、天翼云盘账号和目标目录即可。RSS 来源、过滤正则和集数规则默认收起；已有 RSS 地址时再使用 **手动添加 RSS**。
...
PT 设置支持自定义 Mikan 地址，默认使用 `https://mikanani.kas.pub`。该地址同时用于番剧搜索、字幕组获取和已有 Mikan RSS 拉取。

首选地址不可用时，系统会依次尝试 `https://mikanani.kas.pub`、`https://mikanime.tv`、`https://mikan.sakuramoe.dev` 和 `https://mikan.tangbai.cc`。设置页可以在保存前测试地址连通性。
...
### 8.1 网盘、STRM 与 CAS 目录

开启 `pt.strmOrganize.enabled` 后，整理不再只作用于 STRM：PT 文件会在上传前完成一次正则或 AI+TMDB 解析，并直接上传到相同的媒体库结构。`pt.enableStrm` 只控制是否生成本地 STRM，不影响网盘整理。

```text
目标目录/动漫/作品名 (年份)/Season 01/作品名 - S01E01.mkv
目标目录/_cas/动漫/作品名 (年份)/Season 01/作品名 - S01E01.mkv.cas
```

- 非媒体附件保留原文件名及相对目录，放在作品目录下。
- 同名且 MD5、大小一致的文件直接复用；内容不同则追加字幕组、分辨率或 release ID，避免覆盖。
- `_cas` 是保留目录，不参与媒体整理、STRM 生成和集数统计。
- 配置只影响后续上传和重试，不自动迁移历史 release。
