...
普通转存任务勾选“归档保留 CAS 存根”后，系统先恢复媒体并完成可选的自动整理，再把原 `.cas` 移动到任务所选目标目录下的 `_cas` 镜像树。例如：

```text
目标目录/动漫/作品名/Season 01/E01.mkv
目标目录/_cas/动漫/作品名/Season 01/E01.mkv.cas
```

归档状态会记录到任务处理明细，重启或后续任务执行时会继续补偿失败项。归档失败不会删除原存根。`_cas` 不参与媒体扫描、整理、STRM、集数统计和空目录清理。
