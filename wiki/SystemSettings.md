...
**系统** 页管理访问认证、任务调度、Telegram、消息推送、网络代理、影巢完整配置和正则预设等基础配置。自动追剧配置在 **自动追剧 → 追剧设置** 中独立管理。
...
自动追剧配置已移到 **自动追剧 → 追剧设置**，不再混在系统任务设置中。配置仍使用 `task.autoCreate.*`，供 Web、Telegram Bot、订阅和榜单自动任务共用。
...
| 影巢 API 流控 | 可选；开启后所有影巢请求串行，并按 `hdhive.flowControl.minIntervalMs` 控制相邻请求的最小间隔（0～60000 毫秒） |
