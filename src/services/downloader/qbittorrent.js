...
class QbittorrentClient {
    ...
    getConfig() {
        return ConfigService.getConfigValue('pt.downloader', {}) || {};
    }
    ...
    async testConnection() {
        try {
            ...
            return {
                ok: true,
                message: `qBittorrent ${String(versionResponse.body || '').trim()} (API ${String(apiVersionResponse.body || '').trim()})`,
    ...
    async addTorrent(options = {}) {
        const urls = Array.isArray(options.urls)
            ...
            : (options.url ? [options.url] : (options.magnetUrl ? [options.magnetUrl] : (options.torrentUrl ? [options.torrentUrl] : [])));
